/*******************************************************************************
 * File: Timestamp.java Author: Morteza Ansarinia <ansarinia@me.com> Created on: November 9, 2013 Project: No Time
...
public class NtpTimestamp {
	...
	public NtpTimestamp(final byte[] data) {
		...
		for (int i = 0; i < 8; ++i) {
			final int usigned = ( data[i] < 0 ) ? 256 + data[i] : data[i];
			d = 256.0 * d + usigned;
		}
	 ...
	 * @param posixMillis
	 */
	public NtpTimestamp(final long posixTime) {
		final double posixSec = ( posixTime ) / 1000.0;
		this.value = posixSec + POSIX_TIME_CORRECTION;
	}

	public long toPOSIX() {
		final long time = (long) ( ( this.value - POSIX_TIME_CORRECTION ) * 1000 );
		return time;
	}
	...
	public String toString() {
		return new Date(this.toPOSIX()).toString();
	}
