...
public class Civilization implements Serializable, Comparable<Civilization> {
	...
	public List<Technology> getOptimalTechs(int budget) {
		/*
		 * Legacy call to "permuting" solution return getOptimalTechsPerm(budget);
		 */
	 ...
	 * Returns the optimal tech purchase, in terms of maximizing VP, for a given budget.
	 * 
	 * Checks different combinations of Library and Anatomy to find the best discounts even in the situation where both
	 * techs should be taken.
	 * 
	 * @param budget
	 *            Amount of money available to spend
	 * @return List of technologies worth the most VP at the lowest cost
	...
	public List<Technology> getOptimalCheckDiscounted(int budget) {
		...
		if (!this.hasTech(Technology.ANATOMY) && anatomyCost <= budget) {
			...
			availableTechs.sort((Technology t1, Technology t2) -> this.getCost(t2) - this.getCost(t1)); // sort most
																										// expensive to
																										// least
																										// expensive
			List<Integer> costs =
		...
		// Try with both
		if (!this.hasTech(Technology.LIBRARY) && !this.hasTech(Technology.ANATOMY)
				&& libraryCost + anatomyCost <= budget) {
			...
			for (int i = 0; i < availableTechs.size(); i++) {
				...
				if (i != discountedIdx) {
					...
					// Calculate and keep if better
					List<Technology> testOption =
							getOptimalTechsKS(budget - libraryCost - anatomyCost, availableTechs, costs);
					testOption.add(Technology.LIBRARY);
					testOption.add(Technology.ANATOMY);
					int testVP = testOption.stream().mapToInt(t -> t.getVP()).sum();
					int testCost = this.getTotalCost(testOption);
	 ...
	 * 
	 * Uses the 0-1 Knapsack Dynamic Programming algorithm to quickly find the best possible subset (trades memory usage
	 * for CPU time, but even at the upper limit of 7790 budget, this is a very manageable array size to fill).
	 * Optimizes a bit by "rounding" costs to nearest 5.
	 * 
	 * @param budget
	 *            Amount of money available to spend
	 * @param availableTechs
		...
		for (int i = 0; i <= n; i++) {
			for (int c = 0; c <= budget / 5; c++) {
				if (i == 0 || c == 0) { // clear default row
					...
					maxVP[i][c] =
							Math.max(values.get(i - 1) + maxVP[i - 1][c - ( costs.get(i - 1) / 5 )], maxVP[i - 1][c]); // choose
																														// to
																														// take
																														// or
																														// not
